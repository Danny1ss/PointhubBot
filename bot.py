...
def menu():
    ...
    kb.add(
        InlineKeyboardButton(text="💰 Wallet", callback_data="acc"),
...
def admin_menu():
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(
        InlineKeyboardButton(text="💸 Withdraws", callback_data="admin_w"),
        InlineKeyboardButton(text="🧩 Tasks", callback_data="admin_t")
    )
    return kb
    ...
    if " " in m.text:
        ...
        if u[1] < 100:
            return await m.answer(
                f"❌ Minimum withdraw is 100 points\n"
                f"💰 Your balance: {u[1]}"
            )
